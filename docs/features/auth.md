...
- **CSRF desligado não é falha aqui, e isso foi provado, não assumido.** O CodeQL marca `csrf.disable()` como severidade alta (alertas #100 e #101), e a regra está certa na maioria dos sistemas: ela assume sessão por cookie. CSRF depende de **credencial que o navegador anexa sozinho** numa requisição cross-site. Medido com a stack de pé: o login não devolve `Set-Cookie` nenhum, a sessão é `STATELESS` nos dois serviços, não existe `formLogin` nem `httpBasic`, e um `DELETE` levando só cookies responde `401`. O token vai em `Authorization: Bearer`, escrito pelo JavaScript do site, e o atacante não consegue escrevê-lo porque não consegue lê-lo. A premissa está travada por teste (`CsrfPremissaTest`, nos dois serviços), para o dia em que alguém adicionar login por cookie: nesse dia o teste falha, em vez de a conta ser invadida.
